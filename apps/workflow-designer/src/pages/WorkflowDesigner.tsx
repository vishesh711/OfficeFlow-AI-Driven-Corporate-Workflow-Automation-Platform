...
export function WorkflowDesigner() {
    ...
    mutationFn: async (workflowData: Partial<Workflow>) => {
      if (isEditing && currentWorkflow) {
        ...
        return workflowApi.createWorkflow(
          workflowData as Omit<Workflow, 'id' | 'createdAt' | 'updatedAt'>
        );
      }
    },
    onSuccess: (response) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (!nodes.length) {
      alert(
        '⚠️ Please add at least one node to the workflow before saving.\n\nDrag nodes from the left sidebar onto the canvas.'
      );
      return;
    }
    ...
    if (!validationResult.isValid) {
      ...
      validationResult.errors.forEach((error) => {
        const type = error.message.includes('trigger')
          ? 'Trigger Issues'
          : error.message.includes('recipients') || error.message.includes('template')
            ? 'Email Node Issues'
            : error.message.includes('expression')
              ? 'Condition Node Issues'
              : error.message.includes('duration')
                ? 'Delay Node Issues'
                : error.message.includes('action') || error.message.includes('provider')
                  ? 'Identity Node Issues'
                  : error.message.includes('contentType') || error.message.includes('prompt')
                    ? 'AI Node Issues'
      ...
      errorMessage +=
        '💡 TIP: Open the Validation Panel (bottom right) to see detailed errors and click on them to jump to the problem node.';
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setWorkflowMetadata({ ...workflowMetadata, eventTrigger: e.target.value })
                    }
